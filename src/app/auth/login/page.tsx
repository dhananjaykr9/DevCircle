...
const oauthErrors: Record<string, string> = {
    Configuration: "There is a problem with the server configuration. Check your OAuth provider settings.",
    AccessDenied: "Access denied. You may not have permission to sign in.",
    Verification: "The verification link has expired or has already been used.",
    OAuthSignin: "Could not start the OAuth sign-in flow. Try again.",
    OAuthCallback: "Could not complete the sign-in. Check that your OAuth callback URL is correct.",
...
function LoginForm() {
    ...
    const [error, setError] = useState<string | null>(
        errorParam ? (oauthErrors[errorParam] || oauthErrors.Default) : null
    );
            ...
            <div className="glass-card" style={{ width: "100%", maxWidth: 440, padding: 40, position: "relative", zIndex: 1 }}>
                <div style={{ textAlign: "center", marginBottom: 32 }}>
                    ...
                    <p style={{ color: "rgba(240,244,255,0.5)", fontSize: 14 }}>
                        Sign in to connect with your local tech community.
                    </p>
                </div>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSubmit(new FormData(e.currentTarget)); }} style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 24 }}>
                    ...
                    <div style={{ position: "relative" }}>
                        ...
                        <input name="password" type="password" className="input" placeholder="••••••••" required style={{ paddingLeft: 42, width: "100%" }} />
                    </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16, margin: "24px 0" }}>
                    ...
                    <span style={{ fontSize: 12, color: "rgba(240,244,255,0.4)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Or continue with</span>
                    <div style={{ height: 1, flex: 1, background: "rgba(255,255,255,0.1)" }} />
                </div>
                ...
                <div style={{ textAlign: "center", marginTop: 32, fontSize: 13, color: "rgba(240,244,255,0.5)" }}>
                    Don&apos;t have an account? <Link href="/auth/signup" style={{ color: "#f0f4ff", fontWeight: 600, textDecoration: "none" }}>Sign up here</Link>
                </div>
            </div>
        </main>
    );
